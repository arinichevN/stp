...
#define FUN_LIST_INIT(T) int init ## T ## List(T ## List *list, unsigned int n){list->item = (T *) malloc(n * sizeof *(list->item));if (list->item == NULL) {return 0;}(list)->max_length=n;return 1;}
...
#define FUN_RLIST_INIT(T) int init_ ## T ## RList(T ## RList *list, size_t n) {list->max_length=list->length=0;if(n<=0){list->max_length=0;return 1;}size_t sz=n * sizeof *(list->item);list->item = (T *) malloc(sz);if (list->item == NULL) {return 0;}memset(list->item, 0, sz);list->max_length=n;return 1;}
